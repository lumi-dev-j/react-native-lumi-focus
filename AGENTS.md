...
We are building **Lumi Focus**, a cozy Pomodoro timer mobile app using Expo.

The app helps users focus through a calming, visually appealing experience that may include:

- Pomodoro focus sessions
- short and long break sessions
- customizable timer durations
- animated cozy scenes
- ambient sounds
- daily focus tracking
- streak tracking
- basic focus statistics
- scene and sound selection
- authentication
- beautiful mobile-first UI inspired by cozy productivity apps
...
- it represents a clear UI concept like `TimerCard`, `TimerControls`, `SceneCard`, `SoundCard`, or `PrimaryButton`
...
```ts
import rainyRoom from "@/assets/images/rainy-room.png";
import cozyCat from "@/assets/images/cozy-cat.png";
...
- local notifications when requested

Do not install a new timer, audio, animation, or notification library without user approval.

For timer accuracy, prefer calculating the remaining time from an end timestamp instead of relying only on decrementing a value every second.
...
Use hardcoded JSON/TS for scenes, sounds, and initial app configuration.
